...
function ModalCadastrar({ show, onHide, Cadastro }) {
    ...
    useEffect(() => {
        if (show) {
            ...
            TipoExameService.filtrar({ status:1 }).then(setTiposExames).catch(() => setTiposExames([]));
        }
    }, [show]);

    useEffect(() => {
        const verificar = async () => {
            const { ex_paciente_id, ex_tipo_exame_id, ex_data, ex_hora } = formExame;

            if (!ex_paciente_id || !ex_data || ex_data.length < 10 || !ex_hora) {
            ...
            try {
                const result = await ExameService.verificarDuplicidade(ex_paciente_id, ex_tipo_exame_id, ex_data, ex_hora);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFormExame({ ex_paciente_id: '', ex_tipo_exame_id: '', ex_data: '', ex_hora: '' });
            setErros({});
            setExDuplicado(false);
            setMensagem({ tipo: 'success', texto: 'Exame cadastrado com sucesso!' });
            if (Cadastro) Cadastro();
        } catch (error) {
            setMensagem({ tipo: 'danger', texto: error.message || 'Erro ao salvar exame. Tente novamente.' });
        }
    };
        ...
        <Modal show={show} onHide={handleClose} size="lg" centered>
            <Form onSubmit={handleSubmit} noValidate>
                ...
                <Modal.Footer>
                    ...
                    <Button type="submit" variant="success" disabled={exDuplicado}>CADASTRAR</Button>
                </Modal.Footer>
            </Form>
        </Modal>
    );
