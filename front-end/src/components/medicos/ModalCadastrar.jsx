...
function ModalCadastrar({ show, onHide, Cadastro }) {
    ...
    const [form, setForm] = useState({ med_nome: '', med_crm: '', med_especialidade: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setForm({ med_nome: '', med_crm: '', med_especialidade: '' });
            setErros({});
            setMensagem({ tipo: 'success', texto: 'Médico cadastrado com sucesso!' });
            if (Cadastro) Cadastro();
        } catch (error) {
            setMensagem({ tipo: 'danger', texto: error.response?.data?.error || 'Erro ao salvar médico.' });
        }
    };
        ...
        <Modal show={show} onHide={handleClose} size="lg" centered>
            <Form onSubmit={handleSubmit} noValidate>
                <Modal.Header closeButton>
                    <Modal.Title>CADASTRO DE MÉDICO</Modal.Title>
                </Modal.Header>
                <Modal.Body>
